...
The Docker image includes common runtime utilities used by Letta Code, tools, skills, cron jobs, and channel runtime installers: `nodejs`, `npm`, `git`, `python`/`python3`, `curl`, `wget`, `jq`, Unix `cron`, process tools such as `ps`, networking tools such as `ip`, `ss`, `ping`, `dig`, `nslookup`, `lsof`, and `nc`, plus archive/file helpers such as `zip`, `unzip`, `file`, and `less`. The image is Bun-based and sets `LETTA_PACKAGE_MANAGER=bun`, so `letta channels install ...` uses Bun by default with npm available as a compatibility fallback.
